...
namespace ResxWriter
{
    ...
    public partial class frmMain : Form
    {
        ...
        System.Drawing.Font _menuFont = new System.Drawing.Font("Calibri", 12);
        ...
        void frmMain_Shown(object sender, EventArgs e)
        {
            ...
            // Enable double buffering for the main form. Only used for custom animation, if enabled.
            ...
            Logger.Instance.Write($"{System.ComponentModel.LicenseManager.UsageMode} assembly list:", LogLevel.Debug);
            ...
            {
                Logger.Instance.Write($"Glyph table: {ex.Message}", LogLevel.Error);
            }
        ...
        /// <see cref="ListView"/> event.
        /// </summary>
        void lvContents_MouseDown(object sender, MouseEventArgs e)
        {
            var lv = sender as ListView;
            if (lv.SelectedItems.Count <= 0)
                return;
                ...
                if (e.Clicks == 2 && e.Button == MouseButtons.Left)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        ...
                        if (idx != null)
                        {
                            ...
                            {
                                idx.Cast<int>().ForEach(i => { TestAllEncodings(tbFilePath.Text, i + 1, false); });
                                UpdateStatusBar($"All encodings applied to \"{key}\". See log for results.");
                            }
                        }
                    }
                    #endregion
        ...
        #region [Custom Menu Experiment]
        /// <summary>
        /// Handle the clicked menu item from our <see cref="CustomPopupMenu"/>.
        /// </summary>
        void CustomPopupMenu_MenuItemClicked(object sender, EventArgs e)
        {
        ...
        void FilePathOnTextChanged(object sender, EventArgs e)
        {
            ...
            else // If a change happens too quickly we'll clear the error to avoid stuck state.
            {
                _pathErrorProvider.SetError(tbFilePath, String.Empty);
            }
        ...
        void DrawText(string text)
        {
            ...
            {
                TextRenderer.DrawText(g, text, _standardFont, new Point(cbDelimiters.Right + 20, cbDelimiters.Top + 2), SystemColors.Highlight, SystemColors.HighlightText);
            }
        }
        ...
        /// <summary>
        /// Print out the data using all available code pages. The user can deside which looks correct.
        /// </summary>
        /// <remarks>
        /// "ISO-8859-1", "Windows-1252", and "UTF-8" are the most popular character sets.
        /// https://en.wikipedia.org/wiki/ISO/IEC_8859-1
        /// </remarks>
        void TestAllEncodingsUsingInput(string input = "Il s'agit d'un échantillon de période de dépassement à convertir à l'aide de la bibliothèque d'encodages.")
        {
            var encs = Encoding.GetEncodings();
            foreach (var enc in encs)
            {
                Debug.WriteLine(new string('-',150));
                Encoding test = Encoding.GetEncoding(enc.CodePage);
                var samp = test.GetBytes(input);
                Debug.WriteLine($"[INFO] {enc.Name} (CP{enc.CodePage}) => {test.GetString(samp)}");
            }
        }

        /// <summary>
        /// This is a bit unwieldy, but we need to re-load the file each
        /// time with the new encoding to test the conversion process.
        /// </summary>
        void TestAllEncodings(string filePath, int index, bool terminal = false)
        {
            var encs = Encoding.GetEncodings();
            foreach (var enc in encs)
            {
                int lineCntr = 0;
                Encoding encTest = Encoding.GetEncoding(enc.CodePage);
                try
                {
                    using (StreamReader sr = new StreamReader(filePath, encTest))
                    {
                        while (!sr.EndOfStream)
                        {
                            string line = sr.ReadLine(); lineCntr++;
                            if (!string.IsNullOrEmpty(line) && index == lineCntr)
                            {
                                string[] fields = line.Split(_userDelimiter[0]);
                                if (fields.Length >= 2) // Ensure there are at least two fields.
                                {
                                    try
                                    {
                                        var forLog = encTest.GetBytes(fields[1]);
                                        if (terminal)
                                            Debug.WriteLine($"[INFO] {enc.Name} (CP{enc.CodePage}) => {encTest.GetString(forLog)}");
                                        else
                                            Logger.Instance.Write($"{enc.Name} (CP{enc.CodePage}) => {encTest.GetString(forLog)}", LogLevel.Debug);
                                    }
                                    catch (Exception) { }
                                }
                                break;
                ...
                {
                    Logger.Instance.Write($"{ex.Message}", LogLevel.Error);
                }
            }
        }
        #endregion
